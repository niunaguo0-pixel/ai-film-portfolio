...
作品区展示四部短剧和两部广告片，并预留“其他”分类；不展示占位演示项目。视频优先播放仓库内针对网页压缩的 H.264/AAC MP4，外部原片只作为加载失败后的备用源。海报使用按需解码的轻量 WebP，《疾速序章》和《光，即是她》的封面从成片中提取。首屏右侧是 CSS 制作的非作品视觉，不是作品截图或影片封面。
...
页面注册了 Service Worker，用于缓存页面框架与海报；视频保留按需加载和 HTTP Range 请求，避免首屏下载大文件。视频弹窗会显示加载状态，主源失败时自动尝试备用源，并提供手动重试。
